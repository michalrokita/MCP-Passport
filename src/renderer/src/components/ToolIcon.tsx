...
function PassportMark({ className = '' }: { className?: string }): JSX.Element {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" aria-hidden>
      <rect x="4" y="3" width="16" height="18" rx="2" stroke="currentColor" strokeWidth="1.5" />
      <circle cx="12" cy="11" r="3" stroke="currentColor" strokeWidth="1.5" />
      <path d="M9 16h6M10.5 18h3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  )
}
...
export function ToolIcon({ toolId, className = 'h-5 w-5', decorate = true }: ToolIconProps): JSX.Element {
  ...
  return (
    <span className={`relative inline-flex shrink-0 items-center justify-center rounded-md ${className}`}>
      {toolId === 'passport' && (
        <span className="flex h-full w-full items-center justify-center rounded-md bg-gradient-to-br from-accent to-accent-deep text-white">
          <PassportMark className="h-[60%] w-[60%]" />
        </span>
      )}
      {isClaude && (
